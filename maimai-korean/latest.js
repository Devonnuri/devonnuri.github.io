...
(function() {
    ...
    function applyButtonStyle(node, color='blue') {
        ...
        node.style.boxShadow = '0 1px 3px rgba(0,0,0,0.12), 0 1px 2px rgba(0,0,0,0.24), inset 0 1px 3px rgba(0,0,0,0.12), inset 0 1px 2px rgba(0,0,0,0.24)';
    ...
    const navTranslation = ['홈', '플레이어\n데이터', '친구', '동영상 연동', '플레이 기록', '모든 곡 점수', '컬렉션', '랭킹'];
    const navLink = ['home', 'playerData', 'friend', 'movie', 'playLog', 'music', 'collection', 'ranking'];
    ...
    navList.forEach((e, i) => {
        ...
        e.style.boxShadow = '0 3px 6px rgba(0,0,0,0.16), 0 3px 6px rgba(0,0,0,0.23), inset 0 1px 3px rgba(0,0,0,0.12), inset 0 1px 2px rgba(0,0,0,0.24)';
    ...
    if (location.href.match(/.*\/home\/$/)) {
        const buttonSet = document.querySelector('div.status_you div.status_name.clearfix');
        buttonSet.style.marginBottom = '0.7rem';
        buttonSet.style.border = 'none';

        document.querySelectorAll('div.status_you div.status_name.clearfix a').forEach((e, i) => {
            e.removeChild(e.childNodes[0]);
        ...
        const oshirase = document.querySelector('#container > div > div > div.text_c');
        ...
        document.querySelector('form > span.login_title:nth-of-type(1)').innerText = 'SEGA 아이디';
        document.querySelector('form > span.login_title:nth-of-type(2)').innerText = '비밀번호';

        const checkbox = document.querySelector('form input[type="checkbox"]');
        checkbox.nextSibling.nodeValue = 'SEGA ID 기억하기';
        checkbox.nextSibling.nextSibling.nextSibling.nodeValue = '(공공장소에서는 체크해제 해주세요.)';
        ...
        const signUpLink = document.querySelector('#container div.text_c.color1 a:nth-child(3)');
        document.querySelector('#container div.text_c.color1 a:nth-child(3) img').remove();
        ...
        document.querySelector('div.status_you table tr:nth-of-type(1) td:nth-child(2)').style.width = '20%';

        const friendCount = document.querySelector('div.status_you div.status_data div:nth-child(6) span');
        ...
        for (let i=0; i<5; i++) {
            const link = document.querySelector(`#tab5 > div:nth-child(${i+1}) > a`);
            if (link) {
                link.innerText = tabTranslation[i];
            } else {
                document.querySelector(`#tab5 > div:nth-child(${i+1})`).innerText = tabTranslation[i];
            }
        }

        if (location.href.match(/.*\/friend\/((favoriteOn\/)|(favoriteOff\/))?$/)) {
            const icon = document.querySelector('div.status6.clearfix div.text_c p img');
            icon.nextSibling.nodeValue = '친구 ';
            icon.nextSibling.nextSibling.nextSibling.nodeValue = '명 / 100명 ';

            document.querySelector('table.friend_info tr:nth-child(1) td:nth-child(2)').innerText = '친구 후보에서 친구를 신청하면, 친구가 될 수 있습니다.';
            document.querySelector('table.friend_info tr:nth-child(2) td:nth-child(2)').innerText = '최대 친구 수는 100명입니다.';
            document.querySelector('table.friend_info tr:nth-child(3) td:nth-child(2)').innerText = '친구가 되면 maimaiNET 안에서 액티비티나 동영상의 정보를 공유할 수 있게 됩니다.';
            document.querySelector('table.friend_info tr:nth-child(4) td:nth-child(2)').innerText = '최대 즐겨찾기 수는 20명입니다.';

            let latestTime = document.querySelectorAll('div.friend_box div.text_l div:nth-child(3) font.blue');
            if (latestTime.length < 1) {
                latestTime = document.querySelectorAll('div.friend_box div.text_l div:nth-child(4) font.blue');
            }

            latestTime.forEach(e => {
                e.innerText = '마지막 접속 시간 ';
                e.nextSibling.nodeValue = e.nextSibling.nodeValue.replace('以内', '이내').replace('日', '일 ').replace('月', '달 ').replace('時間', '시간 ').replace('分', '분 ');
            });
            ...
            document.querySelectorAll('div.friend_box div.text_r form button').forEach(e => {
                e.innerText = '즐겨찾기 해제';
            });
            ...
            document.querySelector('table.friend_info tr:nth-child(1) td:nth-child(2)').innerText = '같이 플레이하면 친구 후보로 등록됩니다.';
            document.querySelector('table.friend_info tr:nth-child(2) td:nth-child(2)').innerText = '친구 후보는 최근 플레이한 20명까지 표시됩니다.';
            document.querySelector('table.friend_info tr:nth-child(3) td:nth-child(2)').innerText = '한번에 10건까지 친구를 신청할 수 있습니다.';
        } else if (location.href.match(/.*\/friend\/friendInvite\/$/)) {
            const icon = document.querySelector('div.status6.clearfix div.text_c p img');
            icon.nextSibling.nodeValue = '보낸 신청 ';

            document.querySelector('table.friend_info tr:nth-child(1) td:nth-child(2)').innerText = '당신이 친구 신청한 플레이어들입니다.';
            document.querySelector('table.friend_info tr:nth-child(2) td:nth-child(2)').innerText = '신청은 한번에 10건까지 가능합니다.';
            document.querySelector('table.friend_info tr:nth-child(3) td:nth-child(2)').innerText = '상대가 승인하면 친구로 등록됩니다.';

            document.querySelectorAll('form button[name="invite_cancel"]').forEach(e => {
                e.innerText = '신청 취소';
            });
            ...
            document.querySelector('table.friend_info tr:nth-child(1) td:nth-child(2)').innerText = '당신에게 친구 신청한 플레이어들입니다.';
            document.querySelector('table.friend_info tr:nth-child(2) td:nth-child(2)').innerText = '당신이 승인하면 친구로 등록됩니다.';

            document.querySelector('div.text_c div.status6.clearfix span').innerText = '※ 현재 받은 친구 신청이 없습니다.';
        } else if (location.href.match(/.*\/friend\/friendVs\/.*$/)) {
            const icon = document.querySelector('div.status6.clearfix div.text_c p img');
            icon.nextSibling.nodeValue = '친구 VS ';

            document.querySelector('table.friend_info tr:nth-child(1) td:nth-child(2)').innerText = '즐겨찾기한 친구와 점수를 비교할 수 있습니다.';
            document.querySelector('table.friend_info tr:nth-child(2) td:nth-child(2)').innerText = 'MURASAKi PLUS 이전 버전을 플레이하는 친구와는 점수를 비교할 수 없습니다.';
            ...
            genreBtn.style.background = 'linear-gradient(-20deg, #fc6076 0%, #ff9a44 100%)';
            ...
            levelBtn.addEventListener('click', function () {
                ...
                levelBtn.style.background = 'linear-gradient(-20deg, #fc6076 0%, #ff9a44 100%)';
            });

            document.querySelector('select[name="genre"] option[value="99"]').innerText = '모든 장르';
            document.querySelector('select[name="genre"] option[value="0"]').innerText = 'POPS & 애니메이션';
            document.querySelector('select[name="genre"] option[value="1"]').innerText = 'niconico & 보컬로이드';
            document.querySelector('select[name="genre"] option[value="2"]').innerText = '동방프로젝트';
            document.querySelector('select[name="genre"] option[value="4"]').innerText = '게임 & 버라이어티';
            document.querySelector('select[name="genre"] option[value="5"]').innerText = '오리지널 & 조이폴리스';
            ...
            startButton.style.background = 'linear-gradient(to top, #f77062 0%, #fe5196 100%)';
            ...
            record.innerText = record.innerText.replace('勝', '승').replace('敗', '패').replace('分', '무');
        }
    } else if (location.href.match(/.*\/movie.*$/)) {
        if (document.querySelector('#tab div:nth-of-type(1)').classList.contains('tab_2_on')) {
            document.querySelector('div.status_home.clearfix div:nth-child(1) font.name2').innerText = '※ maimaiNET에 저장된 동영상은 30일간 지나면 자동적으로 삭제됩니다';
            document.querySelector('div.status_home.clearfix div:nth-child(4) div font').innerText = '※ 저장된 동영상이 없습니다.';
        } else {
            document.querySelector('div.status_home.clearfix div:nth-child(1) font.name2').innerText = '※ 니코니코 동영상에 업로드된 동영상입니다.\n※ 썸네일을 클릭하면, 니코니코 동영상으로 이동합니다.';
            document.querySelector('div.status_home.clearfix div:nth-child(4) div font').innerText = '※ 업로드된 동영상이 없습니다.';
        }
        ...
        wordBtn.addEventListener('click', function () {
            genreBtn.style.background = 'rgb(100, 100, 100)';
            levelBtn.style.background = 'rgb(100, 100, 100)';
            wordBtn.style.background = 'linear-gradient(-20deg, #fc6076 0%, #ff9a44 100%)';
            versionBtn.style.background = 'rgb(100, 100, 100)';
        });
        ...
        versionBtn.addEventListener('click', function () {
            ...
            levelBtn.style.background = 'rgb(100, 100, 100)';
            wordBtn.style.background = 'rgb(100, 100, 100)';
            versionBtn.style.background = 'linear-gradient(-20deg, #fc6076 0%, #ff9a44 100%)';
        });
    } else if (location.href.match(/.*\/collection.*$/)) {
        if (location.href.match(/.*\/collection\/$/)) {
            document.querySelector('div.status5 div.text_box').innerText = '※ 특별한 조건이 있는 콜렉션은, 획득했을 때에 표시됩니다.';
        } else if (location.href.match(/.*\/collection\/(icon|trophy|namePlate|frame)\/?$/)) {
            ...
            document.querySelectorAll('div.status5 div.text_c span.name').forEach(e => {
            ...
            document.querySelectorAll('#accordion ul.list').forEach(e => {
                ...
                if ((img = e.querySelector('li:nth-child(1) img'))) {
                    img.nextSibling.nodeValue = img.nextSibling.nodeValue.replace('で交換', '와 교환');
                }
            });
        }
    }
    ...
    const translation = {
        ...
        playerData: {
            ...
            'div.status_you table tr:nth-of-type(11) td:first-child': 'VS 모드 승리 횟수',
            'div.status_you table tr:nth-of-type(12) td:first-child': 'SYNC 모드 플레이 횟수',
            'div.status_you table tr:nth-of-type(13) td:first-child': 'SYNC 모드 도와준 수',
            'div.status5 > p': '난이도별 플레이어 데이터',
        },
        movie: {
            'div.status:nth-of-type(1) div.text_c': '니코니코 동영상에 업로드할 수 있는 동영상은\n하루에 최대 5개입니다.',
            'div.status:nth-of-type(2) div.text_c': '※업로드된 동영상은, 다음과 같은 경우 자동으로 삭제됩니다.',
            'div.status:nth-of-type(2) font b': '・투고한 다음날을 1일째로 세어 90일째 까지 100 재생에 이르지 못하는 경우',
            '#tab > div:nth-child(1) > a': '저장한 동영상',
            '#tab > div:nth-child(2) > a': '업로드한 동영상',
            'div.text_c div:nth-child(9) div:nth-child(1)': '니코니코 동영상에 업로드',
            'div.text_c div:nth-child(9) div:nth-child(3) font': '・maimaiNET에 업로드한 동영상은 니코니코 동영상에 업로드할 수 있습니다.\n・아래의 링크에서, 자신의 니코니코 동영상 ID와 패스워드를 입력하고, 연동해주시기 바랍니다.'
        },
        playLog: {
            'div > p.text_c': '플레이 기록',
        },
        music: {
            ...
            'select[name="word"] option[value="6"]': '하히후헤호(바비부베보,파피푸페포)',
    ...
    for (let url in translation) {
        if (url == 'all' || location.href.match(/(\/[^?]+).*/)[1].match(new RegExp(`.*/${url}.*$`))) {
